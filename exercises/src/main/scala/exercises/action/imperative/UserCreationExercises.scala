...
//  }
  ...
  def readYesNo(line: String): Boolean = line match {
    case "Y" => true
    case "N" => false
  ...
  def readUser(console: Console, clock: Clock, maxAttempt: Int = 3): User = {
    val name = readName(console)
    val date = readDateOfBirthRetry(console, maxAttempt)
    val subscribe = readSubscribeToMailingListRetry(console, maxAttempt)
    User(name, date, subscribe, clock.now())
  }
  ...
  def readSubscribeToMailingListRetry(console: Console, maxAttempt: Int): Boolean = {
    retry(maxAttempt) {
      console.writeLine("Would you like to subscribe to our mailing list? [Y/N]")
      val line = console.readLine()
      onError(readYesNo(line), ex => console.writeLine(ex.getMessage))
    }
  }
