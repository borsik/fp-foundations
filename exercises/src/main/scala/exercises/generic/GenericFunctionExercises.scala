...
import exercises.action.fp.console.UserCreationService
...
object GenericFunctionExercises {
  ...
  case class Pair[A](first: A, second: A) {
    ...
    // def map3[B, C, To](other: Pair[B])(another: Pair[C])(combine: (A, B, C) => To): Pair[To] = {
    //   val zz = zipWith(other)(combine(first, other.first, _))
    // }
      //Pair(combine(first, other.first, another.first), combine(second, other.second, another.second))      
  }
  ...
  lazy val decoded: Pair[String] = secret.swap.map(bytes => new String(bytes.reverse.toArray))
  ...
  case class Predicate[A](eval: A => Boolean) {
    ...
    def contramap[To](zoom: To => A): Predicate[To] = Predicate(value => eval(zoom(value)))
  }
  ...
  lazy val isValidUser: Predicate[User] =
    by(_.name)(Predicate[String](name => name == name.capitalize) && Predicate[String](_.length() >= 3)) && by(_.age)(Predicate[Int](_ > 18))
    //Predicate[User](_.age > 18) && Predicate[User](_.name.length() >= 3) && Predicate[User](user => user.name == user.name.capitalize)

  def by[To](f: User => To)(p: Predicate[To]): Predicate[User] = p.contramap(f)
  ...
  trait JsonDecoder[A] {
    ...
    def map[To](update: A => To): JsonDecoder[To] = (json: Json) => update(decode(json))
    def orElse(anotherDecoder: JsonDecoder[A]): JsonDecoder[A] = (json: Json) => Try(decode(json)).getOrElse(anotherDecoder.decode(json))
  }
  ...
  lazy val userIdDecoder: JsonDecoder[UserId] = new JsonDecoder[UserId] {
    def decode(json: Json): UserId = intDecoder.map(UserId.apply).decode(json)
  }
  ...
  lazy val localDateDecoder: JsonDecoder[LocalDate] =
    (json: Json) => stringDecoder.map(LocalDate.parse(_, DateTimeFormatter.ISO_LOCAL_DATE)).decode(json)
  ...
  val longDateDecoder: JsonDecoder[LocalDate] = (json: Json) => LocalDate.ofEpochDay(json.toLong)

  lazy val weirdLocalDateDecoder: JsonDecoder[LocalDate] = localDateDecoder.orElse(longDateDecoder)
