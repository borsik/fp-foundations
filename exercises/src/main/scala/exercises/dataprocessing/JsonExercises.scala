...
object JsonExercises {
  ...
  //  }                                           }
  //}                                           }
  def anonymize(json: Json): Json = json match {
    case JsonNumber(_) => JsonNumber(0)
    case JsonString(_) => JsonString("***")
    case JsonObject(obj) =>
      JsonObject(obj.map { case (str, json) => str -> anonymize(json) })
  }
  ...
  def depth(json: Json): Int = json match {
    case _: JsonNumber | _: JsonString => 0
    case JsonObject(obj) => obj.values.map(depth(_) + 1).maxOption.getOrElse(0)
  }
