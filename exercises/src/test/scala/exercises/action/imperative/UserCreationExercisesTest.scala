...
class UserCreationExercisesTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  def localDateGen: Gen[LocalDate] = {
    val rangeStart = LocalDate.of(1900, 1, 1).toEpochDay
    val rangeEnd = LocalDate.of(2022, 1, 1).toEpochDay
    Gen.choose(rangeStart, rangeEnd).map(i => LocalDate.ofEpochDay(i))
  }

  test("readSubscribeToMailingList example") {
    forAll { yesNo: Boolean =>
      val inputs = ListBuffer(if (yesNo) "Y" else "N")
  ...
  test("readUser example") {
    ...
    val console = Console.mock(inputs, outputs)
    val now = Instant.now()
    val result  = readUser(console, Clock.constant(now), maxAttempt = 2)
